...
//初始化网络：加载库、创建套接字、绑定IP地址、监听、创建一个接受连接的线程（一直接受连接，不是只为一个客户端服务，而且接受连接还是一个阻塞函数，所以我们为接受连接单独创建一个线程）
bool TcpServerNet::InitNet() {
    ...
    m_sockListen = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);//更改为TCP的套接字，socket不只是这个函数使用，所以定义为类成员
    ...
    //3、选地址--绑定IP地址bind()
    //服务端绑定IP地址的目的是为了在网络中为其服务提供一个明确的、可识别的位置,以便客户端能够找到并与之建立连接。这个过程并不涉及绑定客户端地址,因为客户端地址是由客户端在发起连接时动态指定的。
    //一台服务器可能拥有多个网卡和多个IP地址。通过绑定特定的IP地址,服务端可以指定使用哪个网卡来提供服务,从而满足不同的网络配置和性能需求。
    ...
    */
    ...
    err = bind(m_sockListen, (sockaddr*)&serverAddr, sizeof(serverAddr));//参数：socket,指向sockaddr结构体的指针（上面定义一个sockaddr结构体），sockaddr结构体的长度,注意&serverAddr先取地址，再强转
...
unsigned _stdcall TcpServerNet::AcceptThread(void* lpVoid) {
    ...
    while (!pThis->m_isStop) {//m_isStop静态成员，使用this对象调用
        //接受连接
       SOCKET sockClient= accept(pThis->m_sockListen, (sockaddr*)&addrClient,&addrClientSize);
       //打印连接成功的客户端的IP地址（只要打印了ip地址，才说明客户端和服务端连接成功了）
       cout << "client ip:" << inet_ntoa(addrClient.sin_addr) <<"connect success"<< endl;//inet_ntoa将ulong类型转换成常见的十进制四等分的IP地址
...
void TcpServerNet::UninitNet() {
    ...
    for (auto ite = m_listThreadHandle.begin(); ite != m_listThreadHandle.end();) {
        ...
        ite = m_listThreadHandle.erase(ite);//erase()移除无效节点，返回值是下一个有效节点（所以不需要++）
    }
    
    // 3、关闭socket（n+1个map中的n个和单独的m_sockListen）
    if (m_sockListen && m_sockListen != INVALID_SOCKET) {//存在而且不是无效
    ...
    for (auto ite = m_mapThredIdToSocket.begin(); ite != m_mapThredIdToSocket.end();) {
        ...
        ite = m_mapThredIdToSocket.erase(ite);//同理也不用++
    }
    // 4、卸载库
    WSACleanup();
	}
...
void TcpServerNet::RecvData() {
    ...
    Sleep(100);
    //1、获取当前线程对应的socket
    //获取线程id（通过id在map中找到对应的socket）
    unsigned int threadId = GetCurrentThreadId();//GetCurrentThreadId获取当前线程的id
    ...
    while (!m_isStop) {
        ...
        else if (10054==WSAGetLastError()) {//手动断开连接的情况（关闭登录注册窗口）
            break;
        }
        else {
